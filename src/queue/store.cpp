...
void BlockStore::load() {
    ...
    // queue.jsonl is append-only history (includes submitted + handed-off hashes).
    // Only use it when blocks.db is missing or unreadable — otherwise a restart
    // would put dropped/submitted rows back into the live queue.
    if (!db_ok) recover_jsonl_unlocked();
}
...
int BlockStore::drop_by_hash(const std::vector<std::string>& hashes) {
    if (hashes.empty()) return 0;
    std::unordered_set<std::string> drop(hashes.begin(), hashes.end());
    drop.erase("");
    ...
    pending_.erase(
        std::remove_if(pending_.begin(), pending_.end(),
                       [this, &drop](const PendingBlock& pb) {
                           if (!drop.count(pb.hit.hash_str)) return false;
                           hash_index_.erase(pb.hit.hash_str);
                           return true;
                       }),
        pending_.end());
    const int n = static_cast<int>(before - pending_.size());
    if (n) save_db_unlocked();
    return n;
}
...
std::unordered_map<std::string, int> BlockStore::pending_by_type(bool resubmission_only) {
    ...
    for (const auto& pb : pending_) {
        if (resubmission_only && pb.queue_reason != "resubmission") continue;
