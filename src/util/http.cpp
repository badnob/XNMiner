...
// Per-thread curl handle so brute /verify reuses TCP instead of 90k handshakes.
...
HttpResponse curl_request(const std::string& method, const std::string& url, const std::string& body,
    ...
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT_MS, static_cast<long>(std::min(t, 4000)));
