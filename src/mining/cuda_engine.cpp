...
void CudaEngine::start() {
    ...
    // Prefer L1 for the large Argon2 global working set on Blackwell.
    ...
    if (cudaGetDeviceProperties(&prop, settings_.device_id) == cudaSuccess) {
        ...
        // Blackwell (and Ampere+): keep persisting-L2 carveout at 0 so random
        // Argon2 ref loads get the full normal L2 (streaming loads do the rest).
        if (prop.persistingL2CacheMaxSize > 0) {
            cudaDeviceSetLimit(cudaLimitPersistingL2CacheSize, 0);
        }
        // Slightly larger pending DMA window helps multi-lane H2D/D2H on 50-series.
        if (prop.major >= 12) {
    ...
    if (settings_.cuda_max_lanes > 0) {
        config_max_lanes_ = std::max(1, std::min(settings_.cuda_max_lanes, vram_lanes));
    } else {
        config_max_lanes_ = vram_lanes;
    }
...
bool CudaEngine::restore_lane_cap_if_cool(int temperature_c, int /*difficulty*/,
    ...
    // Hybrid m=100 is when all 8 lanes are needed. The old high-m= gate
    // refused to restore them after a thermal dip.
