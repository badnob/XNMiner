...
// 4 jobs/CTA (128 thr) is the proven 5090 setting — higher packs blow the register
// budget and launch with CUDA error 701 (too many resources).
...
uint32_t selectJobsPerBlock(const cudaDeviceProp& prop)
{
    ...
    // fails launch_bounds / occupancy on Blackwell (err 701).
    (void)prop;
    return 4;
#endif
}
...
// Blackwell / modern CUDA: streaming loads for Argon2 random ref blocks so the
// large working set does not thrash L2; normal stores for sequential curr writes.
...
{
    // block_l is 1 KiB; allow dynamic shared for full Blackwell pack.
...
void KernelRunner::init(std::size_t batchSize_){
    ...
    // Pinned host buffers — full-speed async DMA on Blackwell.
