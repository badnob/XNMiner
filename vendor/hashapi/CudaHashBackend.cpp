                                        ...
                                        std::size_t batch_size)
{
    ...
    // allocations on WDDM have aborted the process on 5090 desktop.
...
HashApiResult CudaHashBackend::runBatch(const HashApiRequest& request)
{
    ...
    try {
        ...
        if (request.gpu_first_blocks) {
            ...
            // Freeze the keys that were just H2D'd. next_keygen must never be
            // what we pair with this wave's digests (same XEN11, wrong key).
            bound_ready_ = false;
            freezeHashedKeys(key_arena, attempts, key_len);
        ...
        const uint8_t* snap_keys = key_arena;
        if (bound_ready_ && bound_attempts_ == attempts &&
            bound_keys_.size() == attempts * key_len) {
            snap_keys = bound_keys_.data();
        } else if (hashed_attempts_ == attempts &&
                   hashed_keys_.size() == attempts * key_len) {
            snap_keys = hashed_keys_.data();
        }
        PipelinedBatch current =
            snapshotOutputs(request, params, snap_keys, key_len, single_key, attempts);
        bound_ready_ = false;
